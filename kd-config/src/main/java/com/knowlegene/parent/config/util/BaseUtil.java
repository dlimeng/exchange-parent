...
///////////////////数学运算函数集合//////////////////////
     ...
     * 取出括号值
     */
    private static Pattern bracketsPattern = Pattern.compile("(?<=\\()(\\S+)(?=\\))");
    /**
     * 提供精确的加法运算。
     *
     * @param v1 被加数
    ...
    public static boolean isBlankSet(Collection collection){
    ...
    public static String isNotBlankRex(String str){
        String replaceStr = str;
        if(StringUtils.isNotBlank(str)){
            replaceStr = str.replaceAll("\\s*|\t|\n|\r", "");
        }
        return replaceStr;
    }
     ...
     * @return
     */
    public static Instant formatDate(Object date){
        Instant result=null;
        ...
        if(tmp.contains(".")){
            ...
            result = Instant.parse(tmp,BaseUtil.getDateTimeFormatter(format));
        }else{
    ...
    public static Timestamp instantToTimestamp(Object instant){
        Timestamp time=null;
        if(instant !=null){
            Date date = new DateTime(instant).toDate();
    ...
    public static boolean isDate(String strDate) {
        Matcher m = patternDate.matcher(strDate);
        if (m.matches()) {
            return true;
        } else {
            return false;
    ...
    public static Date strToDate(String strDate){
        ...
        Date strtodate = simdate.parse(strDate,pos);
        return strtodate;
    }
    ...
    public static DateTimeFormatter getDateTimeFormatter (String formatStr){
        return DateTimeFormat.forPattern(formatStr);
    }
    ...
    public static String getPreviousMonthEnd(){
        ...
        lastDate.roll(Calendar.DATE, -1);//日
        str = sdf.format(lastDate.getTime());
        return str;
    }
    ...
    public static String getNextMonthEnd(){
        ...
        lastDate.set(Calendar.DATE, 1);//把日期设置为当月第一
        lastDate.roll(Calendar.DATE, -1);//日期回滚天，也就是本月最后一
        str = sdf.format(lastDate.getTime());
        return str;
    }
    ...
    public static boolean isIdentityCardAndInRange(String strIdCard, int begin,
        ...
        // 先判断身份证号码的有效性
        boolean result = BaseUtil.isIdentityCard(strIdCard);
        ...
        if(result){
            ...
            int age = Integer.parseInt(BaseUtil.formatDate(date, "yyyy"))
                    - Integer.valueOf(birthYear).intValue();

            String currentDate = BaseUtil.formatDate(date, "MMdd");
    ...
    public static class ListUtils{
        ...
        public static <T> List<T> getRepeatElements(List<T> list, String repeatFieldName){
            ...
            if(BaseUtil.isBlank(repeatFieldName)){
                return null;
            }
            String get = "get";
    ...
    public static class RegularlyUtils{
        ...
        private static String getErrorMsgTemplate(String errorTemplate){
            if(BaseUtil.isBlank(errorTemplate)){
...
//			boolean checkItemScore = BaseUtil.RegularlyUtils.checkRegex(str, regularlyEnum, params);
//			if (!checkItemScore) {
...
//		}
     ...
     * @return
     */
    public static String getBrackets(String value){
        Matcher mat = bracketsPattern.matcher(value);//此处是中文输入的（）
        while(mat.find()){
            return mat.group();
        }
        return null;
